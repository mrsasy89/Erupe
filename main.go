...
func main() {
	...
	logger.Info(fmt.Sprintf("Starting Erupe (9.2.1-%s)", Commit()))
	...
	if config.ErupeConfig.SignV2.Enabled {
		sv2 := signv2server.NewServer(&signv2server.Config{
			Logger:      logger.Named("signv2"),
			ErupeConfig: config.ErupeConfig,
			DB:          db,
		})
	...
	if config.ErupeConfig.API.Enabled {
