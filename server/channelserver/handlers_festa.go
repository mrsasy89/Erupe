...
func handleMsgMhfInfoFesta(s *Session, p mhfpacket.MHFPacket) {
	...
	if start == 0 || TimeAdjusted().Unix() > int64(start)+2977200 /* Questo è il controllo già esistente per la generazione di nuova festa */ {
		// Esegui il reset dei ticket per tutti i personaggi
		_, err := s.server.db.Exec(`UPDATE characters SET mezfes = NULL`)
		if err != nil {
			s.logger.Error("Failed to reset mezfes tickets for all characters", zap.Error(err))
		} else {
			s.logger.Info("MezFes tickets reset for all characters due to new event cycle.")
		}
	}
